...
export function useVoiceRecorder(onRecording: (audio: Blob) => Promise<void>) {
  ...
  function setRecordingPhase(next: Phase) { phaseRef.current = next; setPhase(next) }

  function stop() {
    if (phaseRef.current === 'requesting') { releaseRequestedRef.current = true; return }
  ...
  async function start() {
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        void callbackRef.current(audio).catch(() => setError('Could not save this recording. Try saving it from the Feed.')).finally(() => setRecordingPhase('idle'))
      }
      recorder.start()
