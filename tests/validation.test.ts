...
describe('AI output validation', () => {
  ...
  it('drops times whose phrase is not in the message', () => {
    const raw = { suggestions: [item({ dueAt: '2026-09-27T09:00', dueEvidence: 'by Monday' })] }
  ...
  it('tolerates punctuation differences in evidence from speech', () => {
    const raw = JSON.stringify({ suggestions: [item({ evidence: 'I need to email Sam' })] })
  ...
  it('rejects a malformed response', () => {
    expect(() => parseSuggestions('not json', 'Email Sam', zone, written)).toThrow()
    expect(() => parseSuggestions({ tasks: [] }, 'Email Sam', zone, written)).toThrow()
